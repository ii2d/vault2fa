<script lang="ts">
  ...
  import { AirGapTransmitModal, AirGapReceiveModal } from '$lib/components/sync';
  import {
  ...
  let isAirGapTransmitOpen = $state(false);
  let isAirGapReceiveOpen = $state(false);
      ...
      <div class="flex-1 overflow-y-auto p-6">
        ...
        {:else if activeTab === 'sync'}
          <div class="space-y-6">
            ...
            <div class="rounded-2xl border border-white/5 bg-zinc-950/60 p-4">
              ...
              {#if !localFileSupported}
                ...
                >
                  ...
                  export/import .vault backups or use GitHub Gist & Air-Gap sync.
                </div>
              {:else if localHandle}
            ...
            <!-- 3. Air-Gapped QR Transfer Card -->
            <div class="rounded-2xl border border-white/5 bg-zinc-950/60 p-4">
              <div class="flex items-start justify-between gap-4">
                <div class="flex items-start gap-3">
                  <div
                    class="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-amber-500/10 text-amber-400"
                  >
                    <Radio class="h-5 w-5" />
                  </div>
                  <div>
                    <h3 class="text-xs font-bold text-white">Air-Gapped Vector QR Transfer</h3>
                    <p class="mt-0.5 text-[11px] text-zinc-400">
                      Transfer your entire encrypted vault between devices completely offline
                      without Wi-Fi, Bluetooth, or cables using high-speed animated QR codes.
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div class="mt-4 flex flex-wrap gap-2">
                <button
                  type="button"
                  onclick={() => (isAirGapTransmitOpen = true)}
                  class="flex flex-1 items-center justify-center gap-2 rounded-xl border border-white/10 bg-zinc-900/90 py-2.5 text-xs font-semibold text-zinc-200 transition hover:bg-zinc-800 hover:text-white"
                >
                  <QrCode class="h-4 w-4 text-amber-400" />
                  <span>Transmit Vault (QR Player)</span>
                </button>

                <button
                  type="button"
                  onclick={() => (isAirGapReceiveOpen = true)}
                  class="flex flex-1 items-center justify-center gap-2 rounded-xl bg-indigo-600 py-2.5 text-xs font-semibold text-white shadow-md shadow-indigo-600/25 transition hover:bg-indigo-500"
                >
                  <Radio class="h-4 w-4" />
                  <span>Receive Vault (Scanner)</span>
                </button>
              </div>
            </div>
          </div>
...
<AirGapTransmitModal isOpen={isAirGapTransmitOpen} onClose={() => (isAirGapTransmitOpen = false)} />

<AirGapReceiveModal
  isOpen={isAirGapReceiveOpen}
  onClose={() => (isAirGapReceiveOpen = false)}
  onMerged={() => {
    checkSyncStatus();
  }}
/>
